...
function registraItem(req,res){
	...
	var item = new Analogoutput(req.body);
...
function actualizaItem(req,res){
	...
	var params = req.body;      
 	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Analogoutput.findByIdAndUpdate(itemId, params, { new: true }, (err, itemUpdated) => { 
		if(err){
			res.status(500).send({
				error: err,
	   ...
	   .exec(
	   		(err, itemsFound) => {
	   			if (err){
	   				...
	   				//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	   				Analogoutput.countDocuments({}, (err,conteo) =>{
	   					res.status(200).send({
								items: itemsFound,
								total: conteo
						});
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Analogoutput.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
