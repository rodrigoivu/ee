...
function registraItem(req,res){
	...
	var item = new Digitalinput(req.body);
	   ...
	   .exec(
	   		(err, itemsFound) => {
	   			if (err){
	   				...
	   				//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	   				Digitalinput.countDocuments({}, (err,conteo) =>{
	   					res.status(200).send({
								items: itemsFound,
								total: conteo
						});
...
function itemUltimo(req,res){
	...
	// Digitalinput.findOne().sort({timestamp: -1}).limit(1).exec(function(err, res){
	// 	    if(err){
	// 	        console.log(err);
	// 	    }
	// 	    else{
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Digitalinput.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
