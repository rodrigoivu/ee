...
function registraItem(req,res){
	...
	var item = new Elementocanvasdi(req.body);
...
function actualizaItem(req,res){
	...
	var params = req.body;      
 	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Elementocanvasdi.findByIdAndUpdate(itemId, params, { new: true }, (err, itemUpdated) => { 
		if(err){
			res.status(500).send({
				error: err,
	   ...
	   .exec(
	   		(err, itemsFound) => {
	   			if (err){
	   				...
	   				//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	   				Elementocanvasdi.countDocuments({}, (err,conteo) =>{
	   					res.status(200).send({
								items: itemsFound,
								total: conteo
						});
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Elementocanvasdi.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
